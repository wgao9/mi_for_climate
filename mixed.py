...
def Mixed_KSG(x,y,k=5):
	...
	for i in range(N):
		kp, nx, ny = k, k, k
		if knn_dis[i] == 0:
			kp = len(tree_xy.query_ball_point(data[i],EPSILON,p=float('inf')))
			nx = len(tree_x.query_ball_point(x[i],EPSILON,p=float('inf')))
			ny = len(tree_y.query_ball_point(y[i],EPSILON,p=float('inf')))
			#print(kp,nx,ny)
		else:
			nx = len(tree_x.query_ball_point(x[i],knn_dis[i]-EPSILON,p=float('inf')))-1
			ny = len(tree_y.query_ball_point(y[i],knn_dis[i]-EPSILON,p=float('inf')))-1
...
def Noisy_KSG(x,y,k=10,noise=0.01):
	assert len(x)==len(y), "Lists should have same length"
	assert k <= len(x)-1, "Set k smaller than num. samples - 1"
	N = len(x)
	if x.ndim == 1:
